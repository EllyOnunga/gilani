...
`;
    ...
    const chatRes = await generateText({
      ...
      prompt: prompt,
      system:
        "You are a helpful KCSE physics tutor. Answer the student's question based on their notes context.",
      temperature: 0.7,
      maxRetries: 1,
    });
    ...
    result.success = true;
    console.log(
      `[User ${index + 1}] ✓ Completed in ${result.totalDurationMs}ms (Embed: ${result.embedDurationMs}ms, DB: ${result.dbDurationMs}ms, Chat: ${result.chatDurationMs}ms). Found ${personalCount} personal chunks.`,
    );
  } catch (err: any) {
    result.totalDurationMs = Date.now() - totalStart;
    ...
    console.error(
      `[User ${index + 1}] ✗ Failed after ${result.totalDurationMs}ms. Error: ${result.error}`,
    );
  }
...
async function main() {
  ...
  const avgTotal =
    successful.length > 0
      ? successful.reduce((sum, r) => sum + r.totalDurationMs, 0) / successful.length
      : 0;
  const avgEmbed =
    successful.length > 0
      ? successful.reduce((sum, r) => sum + r.embedDurationMs, 0) / successful.length
      : 0;
  const avgDb =
    successful.length > 0
      ? successful.reduce((sum, r) => sum + r.dbDurationMs, 0) / successful.length
      : 0;
  const avgChat =
    successful.length > 0
      ? successful.reduce((sum, r) => sum + r.chatDurationMs, 0) / successful.length
      : 0;
  ...
  console.log(
    `Successful Requests:              ${successful.length} / ${CONCURRENT_USERS} (${((successful.length / CONCURRENT_USERS) * 100).toFixed(1)}%)`,
  );
  console.log(
    `Failed Requests:                  ${failed.length} / ${CONCURRENT_USERS} (${((failed.length / CONCURRENT_USERS) * 100).toFixed(1)}%)`,
  );
