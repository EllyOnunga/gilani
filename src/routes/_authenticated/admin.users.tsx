...
async function verifyAdmin(request: Request): Promise<string> {
  ...
  const { data: roleCheck } = await supabaseAdmin.from("user_roles").select("role").eq("user_id", authResult.userId).eq("role", "admin").single();
...
const listProfiles = createServerFn({ method: "GET" }).handler(async () => {
  ...
  return (profilesRes.data ?? []).map((p) => ({ ...p, role: roleMap[p.id] ?? "student", conversation_count: convoCount[p.id] ?? 0 })) as Profile[];
});

const listEscalations = createServerFn({ method: "GET" }).handler(async () => {
  const request = getRequest();
  try { await verifyAdmin(request); } catch { return []; }
  const { data, error } = await supabaseAdmin.from("escalations").select("id, conversation_id, user_id, reason, status, detail, reviewer_id, created_at").order("created_at", { ascending: false }).limit(200);
  if (error) return [];
...
const listPlatformStats = createServerFn({ method: "GET" }).handler(async () => {
  ...
  return { totalConversations: convos.count ?? 0, totalMessages: msgs.count ?? 0, totalNotes: notes.count ?? 0, totalEscalations: escalations.length, openEscalations: escalations.filter((e: any) => e.status === "open" || e.status === "pending").length } as PlatformStats;
});
...
const listFeedback = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const { data: roleCheck } = await supabaseAdmin.from("user_roles").select("role").eq("user_id", authResult.userId).eq("role", "admin").single();
  if (!roleCheck) throw new Error("Forbidden");
  const { data, error } = await supabaseAdmin.from("message_feedback").select("*").order("created_at", { ascending: false }).limit(200);
  if (error) throw new Error(error.message);
  const userIds = [...new Set((data ?? []).map((f: any) => f.user_id).filter(Boolean))];
  let profileMap: Record<string, string> = {};
  if (userIds.length > 0) {
    const { data: profileData } = await supabaseAdmin.from("profiles").select("id, display_name").in("id", userIds);
    profileMap = Object.fromEntries((profileData ?? []).map((p: any) => [p.id, p.display_name]));
  }
  return (data ?? []).map((f: any) => ({ ...f, profiles: { display_name: profileMap[f.user_id] ?? null } })) as unknown as MessageFeedback[];
});

const listNewsletterSubscribers = createServerFn({ method: "GET" }).handler(async () => {
  const request = getRequest();
  try { await verifyAdmin(request); } catch { return []; }
  const { data, error } = await supabaseAdmin.from("newsletter_subscribers").select("id, email, name, status, subscribed_at, unsubscribed_at").order("subscribed_at", { ascending: false });
  if (error) return [];
...
const listRateLimits = createServerFn({ method: "GET" }).handler(async () => {
  const request = getRequest();
  try { await verifyAdmin(request); } catch { return []; }
  const { data, error } = await supabaseAdmin.from("rate_limits").select("key, count, reset_at").order("count", { ascending: false }).limit(100);
  if (error) return [];
...
const listPayments = createServerFn({ method: "GET" }).handler(async () => {
  const request = getRequest();
  try { await verifyAdmin(request); } catch { return []; }
  const { data, error } = await supabaseAdmin.from("payments").select("*").order("created_at", { ascending: false }).limit(200);
  if (error) return [];
  const userIds = [...new Set((data ?? []).map((p: any) => p.user_id).filter(Boolean))];
  let profileMap: Record<string, { display_name: string | null; email: string | null }> = {};
  if (userIds.length > 0) {
    const { data: profileData } = await supabaseAdmin.from("profiles").select("id, display_name, email").in("id", userIds);
...
const updateUserPlan = createServerFn({ method: "POST" })
  .validator(z.object({ userId: z.string(), plan: z.enum(["free", "pro"]), planExpiry: z.string().nullable().optional() }))
  .handler(async ({ data }) => {
    const request = getRequest();
    await verifyAdmin(request);
    const { error } = await supabaseAdmin.from("profiles").update({ plan: data.plan, plan_expiry: data.planExpiry ?? null }).eq("id", data.userId);
    if (error) throw new Error(error.message);
  });
...
const resetUserRateLimit = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const request = getRequest();
    await verifyAdmin(request);
    const { error } = await supabaseAdmin.from("rate_limits").delete().like("key", `${data.userId}:%`);
    if (error) throw new Error(error.message);
  });
...
function AdminUsersPage() {
  ...
  const TABS = [
    ...
    { id: "escalations", label: "Escalations", icon: AlertTriangle, badge: dashboard.platformStats.openEscalations },
  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-3 p-3 sm:p-6 lg:p-10">
      ...
      <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between pb-5 border-b border-border/60 gap-4 text-center sm:text-left">
        <div>
          ...
          <p className="mt-1 text-xs sm:text-sm text-muted-foreground">
            {dashboard.profileState.length} users · {dashboard.platformStats.totalConversations.toLocaleString()} convos · {dashboard.platformStats.openEscalations} escalations
          </p>
        </div>
        <div className="flex items-center justify-center sm:justify-end gap-2.5 mt-1 sm:mt-0">
          <button onClick={() => dashboard.loadDashboardData(false)} disabled={dashboard.refreshing}
            ...
            <RefreshCw className={`h-3.5 w-3.5 ${dashboard.refreshing ? "animate-spin" : ""}`} /> Refresh
          </button>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
        {[
          { label: "Total Users", value: dashboard.profileState.length, Icon: User, color: "text-primary" },
          { label: "Conversations", value: dashboard.platformStats.totalConversations.toLocaleString(), Icon: Activity, color: "text-blue-600" },
          { label: "Notes", value: dashboard.platformStats.totalNotes.toLocaleString(), Icon: BookOpen, color: "text-purple-600" },
          { label: "Escalations", value: dashboard.platformStats.openEscalations, Icon: AlertTriangle, color: "text-red-500" },
        ].map(({ label, value, Icon, color }) => (
          <div key={label} className="rounded-lg border border-border bg-card p-2.5 sm:p-4 shadow-sm">
            <div className="flex items-center justify-between mb-1.5 sm:mb-2">
              <p className="font-mono text-[9px] sm:text-[10px] uppercase tracking-widest text-muted-foreground leading-tight">{label}</p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
        {[
          { label: "Satisfaction", value: `${dashboard.satisfactionPct}%`, Icon: ThumbsUp, color: "text-green-600" },
          { label: "Unread", value: dashboard.unreadCount, Icon: Mail, color: "text-amber-600" },
          { label: "Rate Limits", value: dashboard.rateLimits.reduce((a, r) => a + r.count, 0), Icon: RefreshCw, color: "text-orange-500" },
          { label: "Messages", value: dashboard.platformStats.totalMessages.toLocaleString(), Icon: MessageSquare, color: "text-teal-600" },
        ].map(({ label, value, Icon, color }) => (
          <div key={label} className="rounded-lg border border-border bg-card p-2.5 sm:p-4 shadow-sm">
            <div className="flex items-center justify-between mb-1.5 sm:mb-2">
              <p className="font-mono text-[9px] sm:text-[10px] uppercase tracking-widest text-muted-foreground leading-tight">{label}</p>
      ...
      <div className="flex gap-1.5 sm:gap-2 pb-2 overflow-x-auto scrollbar-none snap-x snap-mandatory">
        {TABS.map((t) => {
          const Icon = t.icon;
          return (
            <button key={t.id} onClick={() => dashboard.setTab(t.id as any)}
              className={`snap-start flex items-center gap-1.5 px-3 py-2 sm:px-4 sm:py-2.5 text-[10px] sm:text-xs font-bold font-mono uppercase tracking-wider rounded-xl border transition-all whitespace-nowrap flex-shrink-0 min-h-[36px] sm:min-h-[40px] ${
                dashboard.tab === t.id ? "border-primary text-primary bg-primary/5 font-extrabold shadow-sm" : "border-border/60 text-muted-foreground bg-transparent hover:text-foreground hover:border-border hover:bg-accent/30"}`}>
