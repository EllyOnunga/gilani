...
import { STATUS_META, formatDate } from "@/components/admin/types";
...
export function AdminMessagesTab({ messages, expandedMsg, setExpandedMsg, updatingMsg, handleStatusChange }: Props) {
  return (
      ...
      {messages.map((m) => {
        ...
        return (
          <div key={m.id} className={`rounded-xl border bg-card shadow-sm overflow-hidden transition-colors ${m.status === "unread" ? "border-primary/30" : "border-border"}`}>
            <div className="flex items-start gap-3 px-3 py-3 sm:px-5 sm:py-4 cursor-pointer hover:bg-accent/20 transition-colors" onClick={() => setExpandedMsg(isExpanded ? null : m.id)}>
