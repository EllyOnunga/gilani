  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p
          className={`text-xs font-medium leading-tight transition-colors duration-300 ${state === "waiting" ? "text-muted-foreground" : "text-foreground"
            }`}
        >
          {label}
        </p>
...
export function QuizzesPage({ topicFromUrl }: QuizzesPageProps) {
  const { user } = useAuth();
  ...
  // Fix: Prevent race condition when user clicks "Cancel"
  ...
  const { secondsLeft, isDaily, maxSeconds } = useRateLimitCountdown(
    isRateLimited ? quizError : null,
  );
  ...
  const startQuiz = async () => {
    ...
    try {
      ...
      if (message.toLowerCase().includes("limit")) {
        toast.error(message, {
          action: {
            label: "Upgrade",
            onClick: () =>
              window.dispatchEvent(new CustomEvent("custom:open-plans")),
          },
        });
      } else {
        toast.error(message);
      ...
      if (!generationAbortRef.current) setIsGenerating(false);
    }
  };
  ...
  const finishQuiz = useCallback(async () => {
    ...
    const fallbackTopic = activeTopic.includes(" — ")
      ? activeTopic.split(" — ")[1]
      : activeTopic;
  ...
  }, [userAnswers, questions, quizId, activeTopic, user]);
      ...
      <div className="mx-auto max-w-2xl space-y-5 p-4 sm:p-6 lg:p-10">
        <header className="animate-in-slide text-center sm:text-left">
          ...
          <p className="mt-2 text-sm text-muted-foreground text-center sm:text-left">
            GilaniAI generates tailored MCQ questions and tracks your weak
            topics.
          </p>
        </header>
        ...
        <div className="rounded-xl border border-border bg-card p-4 sm:p-6 shadow-sm space-y-5">
          {quizError && (
            <div
              className={`rounded-xl border overflow-hidden animate-in-slide ${isRateLimited
            ...
            >
              <div className="flex items-start gap-2.5 px-4 py-3">
                ...
                <div className="flex-1 min-w-0">
                  <p
                    className={`text-xs font-semibold ${isRateLimited
                  ...
                  <p
                    className={`text-[11px] mt-0.5 ${isRateLimited
                  ...
                  >
                    {isRateLimited
                      ? isDaily
                        ? `You've used your daily quiz allowance.${secondsLeft > 0
                          ? ` Resets in ${formatTime(secondsLeft)}.`
                          : " Resets at midnight (EAT)."
                        }`
                        : `You're generating quizzes too fast. Take a short break.${secondsLeft > 0
                          ? ` Try again in ${formatTime(secondsLeft)}.`
                          : ""
                        }`
                      : quizError}
                    {!isRateLimited &&
                      " If this keeps happening, reduce question count or try a different topic."}
                  </p>
                </div>
                    ...
                    <button
                      onClick={() =>
                        window.dispatchEvent(
                          new CustomEvent("custom:open-plans"),
                        )
                      }
              ...
              {isRateLimited && secondsLeft > 0 && (
                <div className="h-0.5 bg-amber-200/50 dark:bg-amber-800/50">
                  <div
                    ...
                    style={{
                      width: `${secondsLeft > 0 && maxSeconds > 0
            ...
            <div className="grid grid-cols-4 gap-2">
              {[5, 10, 15, 20].map((n) => (
                <button
                  ...
                  className={`rounded-lg border py-2 sm:py-2.5 text-sm font-bold transition-colors ${questionCount === n
  ...
  if (phase === "loading") {
    const steps = [
      { label: "Checking query context", detail: `Aligning to topic` },
      {
        ...
        detail: `${questionCount} MCQs on ${activeTopic}`,
      },
      {
        label: "Adding distractors",
        detail: "Based on common student mistakes",
      },
      {
        label: "Writing explanations",
        detail: "With Kenyan context where relevant",
      },
      {
        label: "Validating answers",
        detail: "Checking difficulty distribution",
      },
    ];
    ...
    return (
      <div className="mx-auto max-w-2xl space-y-5 p-4 sm:p-6 lg:p-10 text-center">
        ...
        <div className="rounded-xl border border-border bg-card p-4 sm:p-8 shadow-sm">
          ...
          <p className="font-serif text-2xl sm:text-4xl lg:text-5xl font-bold text-primary">
            {pct}%
          </p>
          <p className="mt-2 text-sm text-muted-foreground text-center sm:text-left">
        ...
        <div className="rounded-xl border border-border bg-card p-3 sm:p-5 shadow-sm text-left space-y-3">
          ...
          {questions.map((q, i) => {
            ...
            return (
              <div
                key={i}
                className={`flex gap-2.5 p-3 rounded-xl ${correct
  ...
  // ── Quiz screen ─────────────────────────────────────────────────────────────
  const progress =
    ((current + (answered[current] ? 1 : 0)) / questions.length) * 100;
      ...
      {q && (
        <div className="rounded-xl border border-border bg-card p-4 sm:p-6 shadow-sm space-y-3 sm:space-y-4">
          <div className="flex items-start gap-2 flex-wrap">
            ...
            {q.difficulty && (
              <span
                className={`text-[10px] px-2 py-0.5 rounded-full font-mono uppercase flex-shrink-0 ${q.difficulty === "easy"
                    ? "bg-green-100 text-green-700"
                    : q.difficulty === "hard"
                      ? "bg-red-100 text-red-700"
                      : "bg-yellow-100 text-yellow-700"
          ...
          <div className="space-y-3">
            {q.options.map((opt, i) => {
              let cls =
                "w-full text-left rounded-xl border px-4 py-3.5 sm:py-3 text-sm transition-all duration-150 ";
              if (!answered[current]) {
                cls +=
                  selected === i
