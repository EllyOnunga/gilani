...
const FAQ_ITEMS = [
  ...
  },
  {
    ...
    a: "We fully support KCSE, CBC (Grades 7–9), and IGCSE Pearson Edexcel. More curricula are on the roadmap.",
  },
  {
  ...
  },
  {
    ...
    a: "Inside any tutor chat session, click the 'Escalate' button in the chat header. A qualified teacher will review your query.",
  },
  {
...
function ContactPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setStatus("success");
      setForm({ name: "", email: "", subject: "", category: "general" as Category, message: "" });
    } catch {
      setStatus("error");
    }
  };
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="max-w-5xl mx-auto px-6 py-14 grid gap-12 lg:grid-cols-3">
        {/* Info cards */}
        <div className="space-y-5 lg:col-span-1">
          <div className="rounded-xl border border-border bg-card p-5">
            ...
            <p className="text-xs text-muted-foreground leading-relaxed mb-3">
              For general enquiries, account issues, and billing questions.
            </p>
            <a
              href="mailto:support@gilaniai.vercel.app"
              className="text-xs font-medium text-primary hover:underline break-all"
            >
              support@gilaniai.vercel.app
            </a>
          </div>
          ...
          <div className="rounded-xl border border-border bg-card p-5">
            <div className="flex items-center gap-3 mb-3">
              ...
              <p className="text-sm font-semibold">Based in Kenya</p>
            </div>
            <p className="text-xs text-muted-foreground leading-relaxed">
              Nairobi, Kenya 🇰🇪
              <br />
              Building for KCSE &amp; CBC students across the country.
            </p>
          </div>
          ...
          <Link
            to="/dashboard"
            className="block text-xs text-muted-foreground hover:text-primary transition-colors pt-1"
          >
            ← Back to dashboard
          </Link>
        </div>
        ...
        <div className="lg:col-span-2">
          <div className="rounded-xl border border-border bg-card p-7">
            <h2 className="font-serif text-xl text-foreground mb-1">Send a message</h2>
              ...
              <div className="space-y-4">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div>
                    ...
                    <select
                      ...
                      value={form.category}
                      onChange={handleChange}
                      className="w-full rounded-lg border border-border bg-background px-3 py-2.5 text-sm text-foreground focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary transition-colors"
                    >
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full rounded-lg border border-border bg-background px-3 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary transition-colors resize-none"
                  />
                ...
                <button
                  ...
                  disabled={!canSubmit}
                  className="flex w-full items-center justify-center gap-2 rounded-lg bg-primary px-4 py-3 text-sm font-bold text-primary-foreground transition-all hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {status === "sending" ? (
                    <>
      ...
      {/* FAQ */}
      <section className="border-t border-border bg-sidebar px-6 py-14">
        <div className="max-w-2xl mx-auto">
          <p className="font-mono text-xs uppercase tracking-widest text-primary text-center mb-2">FAQ</p>
          <h2 className="font-serif text-2xl text-foreground text-center mb-8">Common questions</h2>
          <div className="space-y-3">
            {FAQ_ITEMS.map((item, i) => (
              <div key={i} className="rounded-xl border border-border bg-card overflow-hidden">
                <button
                  onClick={() => setOpenFaq(openFaq === i ? null : i)}
                  className="w-full flex items-center justify-between px-5 py-4 text-sm font-medium text-foreground hover:bg-black/5 transition-colors text-left"
                >
                  {item.q}
                  <span className="ml-4 text-muted-foreground text-lg leading-none shrink-0">
                    {openFaq === i ? "−" : "+"}
                  </span>
                </button>
                {openFaq === i && (
      ...
      <footer className="border-t border-border px-6 py-6 text-center text-xs text-muted-foreground">
        © {new Date().getFullYear()} GilaniAI · Built for Kenyan students ·{" "}
