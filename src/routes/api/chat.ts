      ...
      POST: async () => {
        ...
        if (latestMessageContent) {
          try {
            ...
            if (chunks && chunks.length > 0) {
              const words = latestMessageContent.toLowerCase().split(/\s+/).filter((w: string) => w.length > 3);
...
${notesContext ? `Use the following curriculum-grounded study notes uploaded by the student as context for your explanations:
=== STUDY NOTES CONTEXT ===
${notesContext}
==========================` : ''}
...
Engage in a friendly, encouraging Swahili-English (Sheng-infused if appropriate) or clear formal language.`;
        ...
        const model = createLovableAiGatewayProvider(LOVABLE_API_KEY).chatModel('google/gemini-3-flash-preview');
        
        const aiMessages = [
          { role: 'system' as const, content: systemPrompt },
          ...(userParts.length
            ? userParts.map((message: any) => ({ role: 'user' as const, content: message.content || '' }))
        ...
        const responseStream = new ReadableStream({
          async start(controller) {
            try {
              ...
              if (!streamError) controller.close();
            } catch (error) {
              streamError = error;
