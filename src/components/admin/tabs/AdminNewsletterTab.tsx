...
import type { NewsletterSubscriber } from "@/components/admin/types";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg border border-border bg-card p-3 sm:p-6 shadow-sm space-y-3">
        <h3 className="font-serif text-lg font-bold flex items-center gap-2"><Send className="h-4 w-4 text-primary" /> Send Newsletter</h3>
        {nlSent ? (
          <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-4 text-center">
            ...
            <button onClick={() => { setNlSent(null); setNlSubject(""); setNlBody(""); }} className="mt-3 text-xs font-mono underline text-muted-foreground">
              Send another
            </button>
          </div>
        ) : (
          <div className="space-y-3">
            <div>
              ...
              <input type="text" placeholder="e.g. 2026 Exam Revision Tips" value={nlSubject} onChange={(e) => setNlSubject(e.target.value)}
            ...
            <div>
              ...
              <textarea rows={6} placeholder="Write your newsletter content here..." value={nlBody} onChange={(e) => setNlBody(e.target.value)}
            ...
            <button disabled={nlSending || !nlSubject || !nlBody} onClick={handleSend}
              ...
              {nlSending ? <><Loader2 className="h-4 w-4 animate-spin" /> Sending...</> : <><Send className="h-4 w-4" /> Send to {activeCount} subscribers</>}
            </button>
          </div>
        )}
      </div>
