...
function JsonNode({ data, name, isLast = true, level = 0 }: JsonNodeProps) {
    ...
    if (data === null) return <span><span className="text-blue-400">{name ? `"${name}": ` : ""}</span><span className="text-zinc-500">null</span>{!isLast && ","}</span>;
    if (typeof data === "string") return <span><span className="text-blue-400">{name ? `"${name}": ` : ""}</span><span className="text-emerald-400">"{data}"</span>{!isLast && ","}</span>;
    if (typeof data === "number" || typeof data === "boolean") return <span><span className="text-blue-400">{name ? `"${name}": ` : ""}</span><span className="text-orange-400">{data.toString()}</span>{!isLast && ","}</span>;
    ...
    if (isEmpty) return <span><span className="text-blue-400">{name ? `"${name}": ` : ""}</span>{openBracket}{closeBracket}{!isLast && ","}</span>;
    ...
    return (
        <div className="flex flex-col font-mono text-[13px] leading-relaxed">
            ...
            >
                ...
                {!expanded && <span className="text-zinc-400">{closeBracket}{!isLast && ","}</span>}
