...
function TutorThreadInner({ authToken }: { authToken: string | null }) {
  ...
  const handleCurriculumChange = async (newVal: string) => {
    ...
    try {
      ...
      const { error } = await supabase.from("profiles").update({ curriculum: newVal }).eq("id", userId);
      if (error) throw error;
  ...
  useEffect(() => {
    const safety = setTimeout(() => {
      setMessagesLoading((prev) => { if (prev) console.warn("[TutorThread] Safety timeout: forcing messagesLoading off"); return false; });
      setThreadsLoading((prev) => { if (prev) console.warn("[TutorThread] Safety timeout: forcing threadsLoading off"); return false; });
    }, 10000);
    ...
    try {
      ...
      if (
        ...
        generateThreadTitleFn({ data: trimmedInput })
          .then((derivedTitle) => {
            supabase.from("conversations").update({ title: derivedTitle }).eq("id", threadId)
              .then(({ error }) => { if (error) console.error("Failed to update thread title:", error); });
            setThreads((prev) => prev.map((t) => (t.id === threadId ? { ...t, title: derivedTitle } : t)));
          })
          .catch(() => {
  ...
  const handleEscalate = async (email?: string) => {
    ...
    try {
      ...
      setTeacherEmail("");
      toast.success(reviewerId ? "Conversation escalated to your teacher!" : "Conversation escalated to available teachers!");
    } catch (err: any) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        ).catch((e) => { console.error("[TutorThread] session timeout:", e); return { data: { session: null }, error: e }; })) as any;

        const session = sessionResult?.data?.session;
        const sessionError = sessionResult?.error;
        if (sessionError) { if (mounted) setThreadsLoadError(`Authentication error: ${sessionError.message || sessionError}.`); return; }

        const userId = session?.user?.id;
        if (!userId) { if (mounted) setThreadsLoadError("Not authenticated. Please sign in."); return; }

        const { data, error } = (await withTimeout(
          Promise.resolve(supabase.from("conversations").select("id,title,updated_at").eq("user_id", userId).order("updated_at", { ascending: false })),
          8000,
          "Database connection timed out",
        )) as any;

        if (error) { if (mounted) setThreadsLoadError(`Failed to load sessions: ${error.message}`); return; }
  ...
  useEffect(() => {
    ...
    if (!threadId) { setMessagesLoading(false); return; }
    setMessagesLoading(true);
    setMessagesLoadError(null);
    setMessages([]);

    timeoutId = setTimeout(() => {
      if (mounted) { setMessagesLoading(false); setMessagesLoadError("Loading timed out."); }
    }, 5000);

    (async () => {
      try {
        const [messagesRes, escalationRes] = await Promise.all([
          supabase.from("messages").select("*").eq("conversation_id", threadId).order("created_at", { ascending: true }),
          supabase.from("escalations").select("status").eq("conversation_id", threadId).order("created_at", { ascending: false }).limit(1).maybeSingle(),
        ]);
        clearTimeout(timeoutId);
        if (!mounted || loadingThreadId !== threadId) return;
        if (messagesRes.error) { setMessagesLoadError(`Database error: ${messagesRes.error.message}`); setMessagesLoading(false); return; }
        ...
        clearTimeout(timeoutId);
        if (mounted) { setMessagesLoadError("Connection failed. Try refreshing."); setMessagesLoading(false); }
      }
    })();
  ...
  useEffect(() => {
    if (!threadId) return;
    const channel = supabase.channel(`escalation-status-${threadId}`)
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "escalations", filter: `conversation_id=eq.${threadId}` },
        (payload) => {
          const newStatus = (payload.new as any)?.status;
          if (newStatus) {
            setEscalationStatus(newStatus);
            if (newStatus === "resolved") toast.success("A teacher has reviewed your conversation and responded!", { duration: 6000 });
            else if (newStatus === "in_review") toast.info("A teacher is now reviewing your conversation.", { duration: 4000 });
          }
        })
      .subscribe();
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`messages-${threadId}`)
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "messages", filter: `conversation_id=eq.${threadId}` },
        (payload) => {
  ...
  const createNewThread = async () => {
    ...
    if (error) { console.error("[TutorThread] create thread error:", error); return; }
  ...
  const exportAsWord = async () => {
    const children: Paragraph[] = [
      ...
      new Paragraph({ children: [new TextRun({ text: `Exported on ${new Date().toLocaleDateString()}`, color: "888888", size: 18 })] }),
      new Paragraph({ text: "" }),
    ];
    messages.forEach((m: any) => {
      const role = m.role === "user" ? "You" : "GilaniAI";
      ...
      const clean = text.replace(/[#*`$]/g, "").trim();
      children.push(
        new Paragraph({ children: [new TextRun({ text: role, bold: true, color: m.role === "user" ? "1E64FF" : "0096FF", size: 20 })] }),
