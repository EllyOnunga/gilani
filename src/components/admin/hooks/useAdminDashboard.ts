...
type ServerFns = {
  ...
  updateMessageStatus: (args: { data: { id: string; status: "unread" | "read" | "resolved" } }) => Promise<void>;
  updateUserPlan: (args: { data: { userId: string; plan: any; planExpiry: string | null } }) => Promise<void>;
...
export function useAdminDashboard(serverFns: ServerFns) {
  ...
  const satisfactionPct = feedback.length > 0
    ? Math.round((feedback.filter((f) => f.vote === 1).length / feedback.length) * 100)
    : 0;

  const filtered = useMemo(() => {
    const q = search.toLowerCase();
    return !q ? profileState : profileState.filter(
      (p) => p.display_name?.toLowerCase().includes(q) || p.email?.toLowerCase().includes(q) || p.role.toLowerCase().includes(q),
    );
  }, [profileState, search]);

  const filteredEscalations = useMemo(
    () => escalationFilter === "all" ? escalations : escalations.filter((e) => e.status === escalationFilter),
  ...
  const filteredForPlans = useMemo(() => {
    ...
    return !q ? profileState : profileState.filter(
      (p) => p.display_name?.toLowerCase().includes(q) || p.email?.toLowerCase().includes(q) || (p.plan ?? "free").toLowerCase().includes(q),
    );
  ...
  return {
    // Data
    loadingData, profileState, messages, feedback, rateLimits, payments, escalations, newsletter, platformStats, refreshing,
    // UI state
    search, setSearch, planSearch, setPlanSearch, rlSearch, setRlSearch,
    tab, setTab, escalationFilter, setEscalationFilter, expandedMsg, setExpandedMsg,
    // Action loading states
    updating, updatingMsg, updatingPlan, resettingLimit,
    // Newsletter
    nlSubject, setNlSubject, nlBody, setNlBody, nlSending, setNlSending, nlSent, setNlSent,
    // Derived
    unreadCount, satisfactionPct,
    filtered, filteredEscalations, filteredRateLimits, filteredForPlans,
    counts, planCounts, mrr, totalRevenue, activeSubs, expiringSoon,
    // Actions
    loadDashboardData, handleRoleChange, handleStatusChange, handlePlanChange, handleResetLimit,
  };
}
