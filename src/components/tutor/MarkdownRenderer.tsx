...
export const MarkdownRenderer = React.memo(function MarkdownRenderer({
  ...
  const componentsWithStreaming = useMemo(() => ({
    ...
    code: ({ inline, children, className, ...props }: any) => {
      ...
      if (isMath) {
        return <KatexRenderer expression={text} displayMode={displayMode && !inline} isStreaming={isStreaming} />;
      }

      if (isChem) {
        return <KatexRenderer expression={`\\ce{${text}}`} displayMode={!inline} isStreaming={isStreaming} />;
      }
