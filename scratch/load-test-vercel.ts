...
async function runSingleUserSession(
  ...
  try {
    ...
    const { error: convoError } = await supabaseAdmin.from("conversations").insert({
      id: convoId,
    ...
    result.success = true;
    console.log(
      `[User ${index + 1}] ✓ Completed: 1st token in ${result.firstTokenMs}ms, Done in ${result.completionMs}ms (HTTP ${response.status})`,
    );
  } catch (err: any) {
...
async function main() {
  ...
  const avgFirstToken =
    successful.length > 0
      ? successful.reduce((sum, r) => sum + r.firstTokenMs, 0) / successful.length
      : 0;
  const avgCompletion =
    successful.length > 0
      ? successful.reduce((sum, r) => sum + r.completionMs, 0) / successful.length
      : 0;
