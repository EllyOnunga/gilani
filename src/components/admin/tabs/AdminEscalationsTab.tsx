...
export function AdminEscalationsTab({ escalations, filteredEscalations, escalationFilter, setEscalationFilter }: Props) {
  return (
              ...
              <tbody>
                {filteredEscalations.map((esc) => (
                  <tr key={esc.id} className="border-b border-border/50 hover:bg-accent/30 transition-colors">
                    ...
                    <td className="px-2 py-2 sm:px-5 sm:py-3 font-mono text-xs text-muted-foreground">{formatDate(esc.created_at)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
